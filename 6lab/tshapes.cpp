...
int main()
{
    ...
    // tShape* shapess[] = {&line, &triangle[0], &triangle[1], &rectangle, &rhomb, &ellipse, &circle};
    ...
    while (window.isOpen())
    {
        ...
        for(int i = 0; i < 7; i++)
        {
            shapes[i].solve_collision(x_size, y_size);
            shapes[i].move();
            shapes[i].solve_rotate_collision(x_size, y_size);
            shapes[i].rotate();
        }

        for(int i = 0; i < 7; i++)
        {
            window.draw(shapes[i].get_object());
        }
